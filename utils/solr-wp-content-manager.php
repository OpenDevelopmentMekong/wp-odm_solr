...
class WP_Odm_Solr_WP_Content_Manager {
  ...
  var $only_en_local_lang = false;
    ...
    wp_odm_solr_log('solr-wp-content-manager __construct');
    ...
    try {
      ...
      wp_odm_solr_log('solr-wp-content-manager __construct Error: ' . $e);
    }
  ...
  function ping_server(){
    ...
    wp_odm_solr_log('solr-wp-content-manager ping_server');
    ...
    try {
      ...
      wp_odm_solr_log('solr-wp-content-manager ping_server Error: ' . $e);
      return false;
    }
    ...
    wp_odm_solr_log('solr-wp-content-manager index_post ' . serialize($post) . ' new index_id will be ' . md5($post->guid));
    ...
    if ( 'publish' === get_post_status( $post->ID ) ):
      ...
      try {
        ...
        wp_odm_solr_log('solr-wp-content-manager index_post Error: ' . $e);
      }
    ...
    wp_odm_solr_log('solr-wp-content-manager post is not pusblished, skipping.');

  }

	function clear_index(){

    wp_odm_solr_log('solr-wp-content-manager clear_index');
    ...
    try {
      ...
      wp_odm_solr_log('solr-wp-content-manager clear_index Error: ' . $e);
    }
  ...
  function delete_post($post_id){
    ...
    wp_odm_solr_log('solr-wp-content-manager delete_post: ' . $post_id);
    ...
    try {
      ...
      wp_odm_solr_log('solr-wp-content-manager delete_post Error: ' . $e);
    }
    ...
    wp_odm_solr_log('solr-wp-content-manager query: ' . $text . " attrs: " . serialize($attrs)  . " control_attrs: " . serialize($control_attrs));
    ...
    try {
      ...
      wp_odm_solr_log('solr-wp-content-manager executing query: ' . serialize($query));
      ...
      wp_odm_solr_log('solr-wp-content-manager query Error: ' . $e);
    }
...
$GLOBALS['WP_Odm_Solr_WP_Content_Manager'] = new WP_Odm_Solr_WP_Content_Manager();

function WP_Odm_Solr_WP_Content_Manager() {
	return $GLOBALS['WP_Odm_Solr_WP_Content_Manager'];
}
